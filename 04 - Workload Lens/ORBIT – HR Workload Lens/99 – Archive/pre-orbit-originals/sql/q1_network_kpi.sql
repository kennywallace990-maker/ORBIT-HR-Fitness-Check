...
-- v2.0 changes: Super Access No Wages → Automation, Workers Comp → Local HR,
--   HC Comment excluded, weekly MP threshold >= 3, PAYCODE_CATEGORY expanded
...
base AS (
    SELECT a.PERSON_NUMBER AS EMPLOYEE_ID, p.FIRST_NAME||' '||p.LAST_NAME AS EMPLOYEE_FULL_NAME,
        ...
        -- v2.0: Super Access No Wages → Automation (excluded downstream)
        -- v2.0: Workers Compensation → Local HR
        ...
        -- v2.0: Added pto paid dur, personal unpd dur, meal break patterns before generic pto/personal
    ...
    WHERE a.rn = 1
      -- v2.0: Added 'Historical Correction Comment' to exclusion list
      AND a.AUDIT_TYPE NOT IN ('Exception Comment', 'Punch Comment', 'Pay Code Edit Comment', 'Historical Correction Comment')
),
-- v2.1: Added 'WFM' to exclusion list because Workforce Management is out of HR scope
