-- ============================================================
-- VALIDATION QUERY — Single-Week HR Dataset (Sun–Sat)
-- Pulls 1 week of filtered HR data for view validation.
-- Export result to CSV.
--
-- Date range: Sun 2026-03-01 through Sat 2026-03-07
--   (Previous week — 02/22-02/28 excluded due to missing data)
--
-- No view dependencies:
--   ✗ V_HWL_BASE       — not required
--   ✗ V_HWL_HR          — not required
--   ✗ V_HWL_WEEKLY_SITE_METRICS — not required
--
-- To change the target week, update CURRENT_WEEK_START below
-- to the Sunday of your desired week.
-- ============================================================

WITH params AS (
    SELECT DATE '2026-03-01' AS CURRENT_WEEK_START  -- Sunday of target week
),
week_window AS (
    SELECT
        CURRENT_WEEK_START                           AS WEEK_START,   -- 2026-03-01 (Sun)
        DATEADD('day', 6, CURRENT_WEEK_START)        AS WEEK_END      -- 2026-03-07 (Sat)
    FROM params
),
...
audit_deduped AS (
    ...
    WHERE PARTITION_DATE BETWEEN (SELECT WEEK_START FROM week_window)
...
non_comment_revisions AS (
    SELECT DISTINCT AUDIT_REVISION_ID, PERSON_NUMBER
    FROM audit_deduped
    WHERE rn = 1
      AND AUDIT_TYPE NOT IN ('Exception Comment', 'Punch Comment', 'Pay Code Edit Comment', 'Historical Correction Comment')
),

orphaned_comments AS (
    SELECT a.*
    FROM audit_deduped a
    WHERE a.AUDIT_TYPE IN ('Pay Code Edit Comment', 'Punch Comment', 'Exception Comment', 'Historical Correction Comment')
      AND a.rn = 1
      AND (TRIM(COALESCE(a.AUDIT_COMMENT_TEXT, '')) <> '' OR TRIM(COALESCE(a.AUDIT_NOTE_TEXT, '')) <> '')
      AND NOT EXISTS (
          SELECT 1
          FROM non_comment_revisions nc
          WHERE nc.AUDIT_REVISION_ID = a.AUDIT_REVISION_ID
            AND nc.PERSON_NUMBER     = a.PERSON_NUMBER
      )
),
...
weekly_missed_totals AS (
    ...
    WHERE EVENT_DATE BETWEEN (SELECT WEEK_START FROM week_window)
                          AND (SELECT WEEK_END   FROM week_window)
    GROUP BY PERSON_ID
),

-- ============================================================
-- Main action rows (excludes comment-only audit types)
-- ============================================================
base AS (
    SELECT
        ...
        CASE
            ...
            WHEN a.AUDIT_TYPE = 'Manager Justified Time' AND a.AUDIT_REVISION_TYPE = 'Add'
        ...
        CASE
            WHEN a.AUDIT_TYPE = 'Pay Code Edit' AND a.AUDIT_REVISION_TYPE = 'Add' THEN TRUE
            WHEN a.AUDIT_TYPE = 'Manager Justified Time' AND a.AUDIT_REVISION_TYPE = 'Add'
                 AND NOT (LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%late%'
                       OR LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%early%'
                       OR LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%ncns%'
                       OR LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%call off%'
                       OR LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%unpd%') THEN TRUE
        ...
        -- Bucket G: Governance
        CASE
            WHEN a.AUDIT_TYPE IN ('Exception Comment', 'Punch Comment', 'Pay Code Edit Comment', 'Historical Correction Comment') THEN TRUE
        ...
        CASE
            ...
            WHEN a.AUDIT_TYPE = 'Pay Code Edit' AND a.AUDIT_REVISION_TYPE IN ('Add', 'Edit')
                 AND (LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%pto%'
                   OR LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%sick%'
                   OR LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%regular%'
                   OR LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%overtime%') THEN TRUE
            ELSE FALSE
        END AS HIGH_RISK_REWORK,

        -- Comment presence (uses linked comments, not raw)
        CASE
            WHEN (COALESCE(cmt.LINKED_COMMENTS, a.AUDIT_COMMENT_TEXT) IS NOT NULL
                  AND TRIM(COALESCE(cmt.LINKED_COMMENTS, a.AUDIT_COMMENT_TEXT)) <> '')
              OR (COALESCE(cmt.LINKED_NOTES, a.AUDIT_NOTE_TEXT) IS NOT NULL
                  AND TRIM(COALESCE(cmt.LINKED_NOTES, a.AUDIT_NOTE_TEXT)) <> '')
            THEN 1 ELSE 0
        END AS HAS_COMMENT,

        -- Paycode Category
        CASE
            WHEN a.AUDIT_PAYCODE_NAME IS NULL OR TRIM(a.AUDIT_PAYCODE_NAME) = '' THEN 'Manual Punch Correction'
            ...
            ELSE 'Time spent manually coding ' || COALESCE(a.AUDIT_PAYCODE_NAME, '')
        END AS PAYCODE_CATEGORY,

        -- HC Category
        CASE
            WHEN LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%ncns%' OR LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%late%' OR LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%early%' OR LOWER(a.AUDIT_PAYCODE_NAME) LIKE '%call off%' THEN 'Attendance Enforcement'
        ...
        CASE
            WHEN p.EMPLOYMENT_STATUS = 'Active'
                 AND (COALESCE(mpc.DAILY_MISSED_PUNCHES, 0) >= 2
                   OR COALESCE(wmt.WEEKLY_MISSED_PUNCHES, 0) >= 3)
            THEN wmt.MISSED_PUNCH_DATES ELSE NULL
        END AS MISSED_PUNCH_DATES,
        exc.EXCEPTION_TYPES

    FROM audit_deduped a
    JOIN people_deduped p
      ON a.PERSON_NUMBER = p.PERSON_NUMBER AND p.prn = 1
    LEFT JOIN people_deduped rev
      ON a.AUDIT_REVISION_USER_PERSON_NUMBER = rev.PERSON_NUMBER AND rev.prn = 1
    LEFT JOIN comments_by_revision cmt
      ON a.AUDIT_REVISION_ID = cmt.AUDIT_REVISION_ID AND a.PERSON_NUMBER = cmt.PERSON_NUMBER
    LEFT JOIN missed_punch_counts mpc
      ON mpc.PERSON_ID = p.PERSON_ID AND mpc.EVENT_DATE = a.PARTITION_DATE
    LEFT JOIN weekly_missed_totals wmt
      ON wmt.PERSON_ID = p.PERSON_ID
    LEFT JOIN (
    ...
    -- ============================================================
    -- Orphaned comments: comment rows whose parent revision has
    -- no non-comment action row. Bucket flags are synthetic.
    -- ============================================================
    SELECT
        a.PERSON_NUMBER AS EMPLOYEE_ID,
        p.FIRST_NAME || ' ' || p.LAST_NAME AS EMPLOYEE_FULL_NAME,
        ...
        -- Synthetic bucket flags for orphaned comments
        TRUE  AS BUCKET_B,
        FALSE AS BUCKET_A,
        TRUE  AS BUCKET_G,
        0.5   AS FRICTION_SCORE,
        FALSE AS HIGH_RISK_REWORK,
        1     AS HAS_COMMENT,
        'Orphaned Comment' AS PAYCODE_CATEGORY,
        'Other' AS HC_CATEGORY,
...
SELECT
    -- Sunday-based week start (session-independent)
    ENTITY_EVENT_DATE - DAYOFWEEK(ENTITY_EVENT_DATE) AS REPORT_WEEK,
    EMPLOYEE_ID,
    BUILDING_LOCATION,
    OBR_SITE_GROUP,
    OBR_ACTOR_GROUP,
    ...
    NOTE_TEXT
FROM hr
ORDER BY OBR_SITE_GROUP, BUILDING_LOCATION, ENTITY_EVENT_DATE;
