-- QUERY 8 — Missed Punch Engagement Opportunities (Section 6 / Engagement List)
-- Standalone query — goes directly to V_TIMECARD_EXCEPTION, NOT through the audit pipeline.
-- This ensures every employee with missed punches is captured, even if no HR audit action exists.
-- Returns 0–500+ rows. 0 rows is valid (no flagged TMs). Never halt on 0 rows.
--
-- v2.0 changes:
--   - Workers Compensation added to Local HR in OBR_SITE_GROUP (consistency with Q1-Q7)
--   - Weekly threshold uses >= 3 (PRD §8), consistent with Q1-Q7
--   - First-week TM hire date filter NOT YET IMPLEMENTED (pending HIRE_DATE confirmation)
--
